(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        chrome.tabs.getSelected(null, function(tab) {
            ...
            Request.loadUrl(url, function(response) {
                ...
                if (threads.length <= 0) l('tabs-container').innerHTML = "Couldn't find any posts for this page!";
    ...
    var Thread = (function() {
        ...
        Thread.prototype.loadComments = function() {
            ...
            var html = "<div class='story' id='" + _this.threadData.id + "'>" +
                "<div class='title'><a target='_blank' href='http://reddit.com" + _this.threadData.permalink + "'> " + _this.threadData.title + "</a></div>" +
                "<div class='comments' id='" + _this.threadData.id + "_comments'>" +
                "</div></div>";

            l('stories').innerHTML = html;
    ...
    function Comment(parentId, comment, parent) {
        ...
        elem.innerHTML += "<div class='comment' id=" + comment.data.id + "><div class='info'><span class='minimizer'>[-]</span><span> " + comment.data.author + " </span></div>" + body + "</div>";
